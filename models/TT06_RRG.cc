...
double TT06_RRG::computeRates(double dt, double V, double iStim, bool integrate)
{
  double algebraic[70];
  double rates[20];
...
class CaOutput : public ModelOutput<TT06>
{
...
public:
  ...
  virtual void output_data(std::ostream& file, TT06& model) {
    file << model.get_calcium();
  }

};

class CaSROutput : public ModelOutput<TT06>
{
...
//output
void TT06::
set_output(const std::vector<std::string>& save)
{

  ModelOutput<TT06>* container[] = {new CaOutput(), new CaSROutput()}; //memory leak so far
  std::map<std::string, ModelOutput<TT06>*> m;
  m["Ca_i"] = container[0];
  ...
  for(unsigned ii = 0; ii < n_save; ii++) {
    std::map<std::string, ModelOutput<TT06>*>::iterator it = m.find(save[ii]);

    if(it == m.end()) {
      throw exceptions::WrongOption() << "Saving option " << save[ii]
                                      << " is not defined for TT06 model!";
    }
...
void TT06::
output_header(std::ostream& file)
{
